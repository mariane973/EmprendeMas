...
import 'package:EmprendeMas/vistas/emprendedores/detalleServOfertaV.dart';
...
class ServiciosOfertaV extends StatefulWidget {
  final String correo;
  const ServiciosOfertaV({required this.correo});

  @override
  State<ServiciosOfertaV> createState() => _ServiciosOfertaVState();
}


class _ServiciosOfertaVState extends State<ServiciosOfertaV> {
  ...
  void _filtradoOferta() async {
    ...
    for (var servicioDoc in serviciosSnapshot.docs) {
      var data = servicioDoc.data();
      data['id'] = servicioDoc.id;
      allResults.add(data);
    }
    setState(() {
  ...
  void searchResultList() {
    ...
    if (_searchController.text.isNotEmpty) {
      for (var servicio in _allResults) {
        var nombre = servicio['nombre'].toString().toLowerCase();
        if (nombre.contains(_searchController.text.toLowerCase())){
          showResults.add(servicio);
        }
      }
    } else {
                                            ...
                                            child: ElevatedButton.icon(
                                              onPressed: () {
                                                Navigator.push(context,
                                                    MaterialPageRoute(builder: (context)=>DetalleServOfertaV(servicio: servicio, uidServicio: servicio['id'], correo: widget.correo)));
                                              },
                                              icon: Icon(
